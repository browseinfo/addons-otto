...
class account_invoice(osv.osv):
    ...
    def get_address(self, cr, uid, context=None):		
        if context is None:
            context = {}
        res = {}
        #print 'si entre se obtiene dirección de la company'
        partner_address_brw = self.pool.get('res.users').browse(cr,uid,uid).company_id.partner_id.address
        for each_brw in partner_address_brw:
			if each_brw.type=='invoice':
				res['street']=each_brw.street
				res['street2']=each_brw.street2		    
		        res['zip']=each_brw.zip		    
		        res['state']=each_brw.state_id.name
		        res['city']=each_brw.city
		        res['phone']=each_brw.phone
		        res['email']=each_brw.email
        return res
